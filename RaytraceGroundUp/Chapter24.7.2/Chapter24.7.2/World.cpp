...
void World::create_reflective_cornell_box(int iWidth, int iHeight)
{
	// 
	Phong * phong_right_ptr = new Phong;
	phong_right_ptr->set_cd(red);
	...
	Reflective * reflective_left_ptr = new Reflective;
	reflective_left_ptr->set_cd(olive);
    reflective_left_ptr->set_ka(0.25) ;
    reflective_left_ptr->set_kd(0.5) ;
    reflective_left_ptr->set_ks(0.15) ;
    reflective_left_ptr->set_exp(100) ;
    reflective_left_ptr->set_kr(0.75) ;
    reflective_left_ptr->set_cr(white) ;
	...
	Reflective * reflective_bottom_ptr = new Reflective;
	reflective_bottom_ptr->set_cd(teal);
    reflective_bottom_ptr->set_ka(0.25) ;
    reflective_bottom_ptr->set_kd(0.5) ;
    reflective_bottom_ptr->set_ks(0.15) ;
    reflective_bottom_ptr->set_exp(100) ;
    reflective_bottom_ptr->set_kr(0.75) ;
    reflective_bottom_ptr->set_cr(white) ;

	Reflective * reflective_back_ptr = new Reflective;
	reflective_back_ptr->set_cd(purple);
    reflective_back_ptr->set_ka(0.25) ;
    reflective_back_ptr->set_kd(0.5) ;
    reflective_back_ptr->set_ks(0.15) ;
    reflective_back_ptr->set_exp(100) ;
    reflective_back_ptr->set_kr(0.75) ;
    reflective_back_ptr->set_cr(white) ;

	Box* box_ptr = new Box(Point3D(0.0), Point3D(1.0));

	Instance* box_back_ptr = new Instance(box_ptr);
	box_back_ptr->set_material(reflective_back_ptr);
	box_back_ptr->scale(iWidth, iHeight, WALL_THICKNESS);
	box_back_ptr->translate(-iWidth / 2, -iHeight / 2, 0.0);
	add_object(box_back_ptr);

	Instance* box_left_ptr = new Instance(box_ptr);
	box_left_ptr->set_material(reflective_left_ptr);
	box_left_ptr->scale(iWidth, iHeight, WALL_THICKNESS);
	box_left_ptr->translate(-iWidth / 2, -iHeight / 2, 0.0);
	box_left_ptr->rotate_y(90);
	box_left_ptr->translate(-iWidth / 2, 0.0, iWidth / 2);
	...
	Instance* box_bottom_ptr = new Instance(box_ptr);
	box_bottom_ptr->set_material(reflective_bottom_ptr);
	box_bottom_ptr->scale(iWidth + WALL_THICKNESS, iWidth, WALL_THICKNESS);
	box_bottom_ptr->translate(-iWidth / 2, -iHeight / 2, 0.0);
	box_bottom_ptr->rotate_x(90);
	box_bottom_ptr->translate(0.0, -iHeight / 2, iWidth / 2 + 2 * WALL_THICKNESS);
...
void World::build(char * file_name)
{
	...
	add_Phong_axis();
	set_back_plane();
	create_reflective_cornell_box(400, 450);
	...
	instance_ptr->translate(0.0, 100.0, 200.0);
	instance_ptr->compute_bounding_box();
	...
	pinhole_ptr->set_lookat(-200, 0, 0);
