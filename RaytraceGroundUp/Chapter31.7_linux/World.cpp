...
void World::build()
{
    ...
    //image：
    Image* image_sandstone_ramp1_ptr=new Image;
    image_sandstone_ramp1_ptr->read_ppm_file("sandstone_ramp1.ppm");
    //image：
    Image* image_sandstone_ramp2_ptr=new Image;
    image_sandstone_ramp2_ptr->read_ppm_file("sandstone_ramp2.ppm");
	//the noise:
	CubicNoise* noise_bunny_ptr = new CubicNoise;
	noise_bunny_ptr->set_num_octaves(2);
	noise_bunny_ptr->set_gain(0.5);
	noise_bunny_ptr->set_lacunarity(2.0);

	//the texture:
	FBmTextureRamp* fBm_texture_bunny_ptr = new FBmTextureRamp(noise_bunny_ptr);
	fBm_texture_bunny_ptr->set_perturbation(0.3);
    fBm_texture_bunny_ptr->set_image(image_sandstone_ramp1_ptr);

	//the noise:
	CubicNoise* noise_box_ptr = new CubicNoise;
	noise_box_ptr->set_num_octaves(6);
	noise_box_ptr->set_gain(0.5);
	noise_box_ptr->set_lacunarity(2.0);

	//the texture:
	FBmTextureRamp* fBm_texture_box_ptr = new FBmTextureRamp(noise_box_ptr);
	fBm_texture_box_ptr->set_perturbation(0.5);
    fBm_texture_box_ptr->set_image(image_sandstone_ramp2_ptr);

	//the material:
	SV_Matte* sv_matte_bunny_ptr = new SV_Matte;
	sv_matte_bunny_ptr->set_ka(0.25);
	sv_matte_bunny_ptr->set_kd(0.85);
	sv_matte_bunny_ptr->set_cd(fBm_texture_bunny_ptr);

	//the material:
	SV_Matte* sv_matte_box_ptr = new SV_Matte;
	sv_matte_box_ptr->set_ka(0.25);
	sv_matte_box_ptr->set_kd(0.85);
	sv_matte_box_ptr->set_cd(fBm_texture_box_ptr);
    ...
    //sphere_ptr->set_material(sv_matte_ptrl);
    //add_object(sphere_ptr);
	...
	Grid* bunny_ptr = new Grid(new Mesh);
	bunny_ptr->read_smooth_triangles(file_name);
	bunny_ptr->set_material(sv_matte_bunny_ptr);
	bunny_ptr->setup_cells();

	Instance* instance_ptr = new Instance;
	//add whole grid up to this level
	instance_ptr->set_object(bunny_ptr);
	instance_ptr->set_material(sv_matte_bunny_ptr);
	...
	Box* box_ptr1 = new Box(Point3D(0.0), Point3D(1.0));
	box_ptr1->set_material(sv_matte_box_ptr);
	Instance* box_ptr = new Instance(box_ptr1);
	// box_ptr->rotate_x(45);
	// box_ptr->rotate_y(45);
	box_ptr->rotate_z(45);
	box_ptr->scale(30, 40, 50);
	box_ptr->translate(130, -30, 10);
	add_object(box_ptr);
