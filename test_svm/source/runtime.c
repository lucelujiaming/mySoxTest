...
{
    list_node_t *_node = (list_node_t *)(ctx.runhours.first);
    unsigned char sum = 0;
...
#endif
        int i;
        for (i=0; i<(int)strlen(hour->name); i++) {
...
#endif
        int i;
        for (i=0; i<(int)strlen(point->name); i++) {
...
#ifdef WIN32
static unsigned __int64 runtime_next_update = 0;
#else
...
static void update_runtime(void)
{
	T_RUNHOUR *hour = NULL;
#ifdef WIN32
    unsigned __int64 curr = get_tick_ms();
#else
    ...
    if (runtime_next_update < curr)
    {
        ...
        while (_node != NULL) {
#ifdef WIN32
			hour = (T_RUNHOUR*)(_node) ; 
#else
            hour = list_entry_safe(_node, T_RUNHOUR);
#endif
            if (hour->en != 0) {
                hour->seconds ++;
...
Cell sys_Runtime_mathExp(SedonaVM* vm, Cell* params)
{
    ...
    float *buf = params[0].aval;
#ifdef WIN32
    buf[1] = (float)pow(buf[0], (int)buf[1]);
#else
...
Cell sys_Runtime_mathExpt(SedonaVM* vm, Cell* params)
{
    ...
    float *buf = params[0].aval;
#ifdef WIN32
    buf[2] = (float)log(buf[0])/(float)log(buf[1]);
#else
