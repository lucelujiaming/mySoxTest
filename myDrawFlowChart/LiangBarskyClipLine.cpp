...
/*     u = Pk / Qk                                  - 公式(15)          */
/************************************************************************/
/* 下面给出Pk的含义。他们的含义其实很好理解。这里只是列出。             */
/* 首先是Qk < 0 的几何意义。                                            */
/*   Q1 = x1 - m_Start.x < 0 => x1 < m_Start.x => P1位于左边界的左侧。  */
/*   Q2 = m_End.x - x1   < 0 => x1 > m_End.x   => P1位于右边界的右侧。  */
/*   Q2 = y1 - m_Start.y < 0 => y1 < m_Start.y => P1位于下边界的下侧。  */
/*   Q2 = m_End.y - y1   < 0 => y1 > m_End.y   => P1位于上边界的上侧。  */
/* 之后是Qk > 0 的几何意义。                                            */
/*   Q1 = x1 - m_Start.x > 0 => x1 > m_Start.x => P1位于左边界的右侧。  */
/*   Q2 = m_End.x - x1   > 0 => x1 < m_End.x   => P1位于右边界的左侧。  */
/*   Q2 = y1 - m_Start.y > 0 => y1 > m_Start.y => P1位于下边界的上侧。  */
/*   Q2 = m_End.y - y1   > 0 => y1 < m_End.y   => P1位于上边界的下侧。  */
/* 可以看出，这个Qk类似于Cohen-Sutherland裁剪算法中的区域编码。         */
/************************************************************************/
