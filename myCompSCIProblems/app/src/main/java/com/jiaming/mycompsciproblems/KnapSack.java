    ...
    public static List<Item> knapSack(List<Item> items, int maxCapacity) {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            for (int capacity = 0; capacity <= maxCapacity; capacity++) {
                ...
                double preItemValue = table[i][capacity];

                // 否则，我们就要考虑在当前容量以内，加入“新”物品能否产生比最后一种物品组合更高的价值。
                if (capacity >= item.weight) {
                    // 表中已算出的价值是指从当前容量中减去该物品重量后的容量对应的最近一次物品组合的价值。
                    double valueFreeWeightForitem = table[i][capacity - item.weight];

                    // 只要将该物品的价值加上表中已算出的价值即可得知，
                    double doubleWeightWithCurrentItem = valueFreeWeightForitem + item.value;
                    // 如果总价值高于当前容量下的最后一种物品组合的价值，就将其插入表中，
                    if(doubleWeightWithCurrentItem > preItemValue) {
                        table[i + 1][capacity] = doubleWeightWithCurrentItem;
                    }
                    // 否则插入最后一种组合的价值：
                    else {
                        table[i + 1][capacity] = preItemValue;
                    }
                }
