...
TorusPartConcave::TorusPartConcave (const double _a,      const double _b,
...
bool
TorusPartConcave::hit(const Ray& ray, double& tmin, ShadeRec& sr) const {
    if (!bbox.hit(ray))
        return (false);
...
bool
TorusPartConcave::shadow_hit(const Ray& ray, float& tmin) const {
    if (!bbox.hit(ray))
        return (false);
