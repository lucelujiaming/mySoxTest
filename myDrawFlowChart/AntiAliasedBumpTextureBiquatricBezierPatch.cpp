...
void CAntiAliasedBumpTextureBiquatricBezierPatch::Draw(CDC* pDC, CAntiAliasedBumpTextureZBuffer* pZBuffer)
{
	...
	for (int nFace = 0; nFace < 81; nFace++)//曲面片划分为81个平面片
	{
		...
		pZBuffer->FillTriangle(pDC, Eye, pScene, pTexture->DestImage);
		//绘制右下三角形
...
void CAntiAliasedBumpTextureBiquatricBezierPatch::LeftMultiplyMatrix(double M[3][3], CColorP3 P[3][3])//左乘矩阵M*P
{
	CColorP3 T[3][3];//临时矩阵
	for(int i = 0;i < 3;i++)
		for(int j = 0;j < 3;j++)
...
void CAntiAliasedBumpTextureBiquatricBezierPatch::LeftMultiplyMatrix(double M[3][3], double W[3][3])//左乘矩阵M*W
{
	double T[3][3];//临时矩阵
	for(int i = 0;i < 3;i++)
		for(int j = 0;j < 3;j++)
