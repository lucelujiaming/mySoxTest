...
void World::build()
{
	...
	openCylinder_ptr1->set_material(glass_ptr);

	Instance* instance_ptr = new Instance;
	//add whole grid up to this level
	instance_ptr->set_object(openCylinder_ptr1);
	instance_ptr->set_material(glass_ptr);
	instance_ptr->scale(30.0, 30.0, 30.0);
	instance_ptr->translate(-100.0, 100.0, 100.0);
	instance_ptr->compute_bounding_box();
	add_object(instance_ptr);

	SpherePartConcave * spherepartconcave_ptr1 = new SpherePartConcave();
	spherepartconcave_ptr1->set_material(glass_ptr);

	instance_ptr = new Instance;
	//add whole grid up to this level
	instance_ptr->set_object(spherepartconcave_ptr1);
	instance_ptr->set_material(glass_ptr);
	instance_ptr->rotate_x(90);
	instance_ptr->scale(30.0, 30.0, 30.0);
	instance_ptr->translate(100.0, 50.0, 200.0);
	instance_ptr->compute_bounding_box();
	add_object(instance_ptr);
    ...
    pinhole_ptr->set_eye(100, -100, 400);
    pinhole_ptr->set_lookat(0, 50, -50);
