...
void World::build()
{
    ...
    FishEye* fish_eys_ptr=new FishEye;

    // We can not see anything from this position and direction
    // fish_eys_ptr->set_eye(0, 6, 50);
    // fish_eys_ptr->set_lookat(0, 6, 0);
    fish_eys_ptr->set_eye(300, 400, 500);
    fish_eys_ptr->set_lookat(0, 0, -50);
    fish_eys_ptr->compute_uvw();
    set_camera(fish_eys_ptr) ;
