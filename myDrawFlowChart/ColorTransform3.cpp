...
void CColorTransform3::Identity(void)//单位矩阵
{
...
void CColorTransform3::Translate(double tx, double ty, double tz)//平移变换
{
	Identity();
	M[0][3] = tx;
	M[1][3] = ty;
...
void CColorTransform3::Scale(double sx, double sy, double sz)//比例变换
{
	Identity();
	M[0][0] = sx;
	M[1][1] = sy;
...
void CColorTransform3::Scale(double sx, double sy, double sz, CColorP3 p)//相对于任意点的比例变换
{
...
void CColorTransform3::RotateX(double beta)//绕X轴旋转变换
{
	Identity();
...
void CColorTransform3::RotateZ(double beta, CColorP3 p)//相对于任意点的绕Z轴旋转变换
{
	Translate(-p.x, -p.y, -p.z);
	RotateZ(beta);
	Translate(p.x, p.y, p.z);
...
void CColorTransform3::ReflectX(void)//X轴反射变换
{
	Identity();
	M[1][1] = -1, M[2][2] = -1;
	MultiplyMatrix();
...
void CColorTransform3::ReflectY(void)//Y轴反射变换
{
	Identity();
	M[0][0] = -1, M[2][2] = -1;
	MultiplyMatrix();
...
void CColorTransform3::ReflectZ(void)//Z轴反射变换
{
	Identity();
	M[0][0] = -1, M[1][1] = -1;
	MultiplyMatrix();
...
void CColorTransform3::ReflectXOY(void)//XOY面反射变换
{
	Identity();
	M[2][2] = -1;
	MultiplyMatrix();
...
void CColorTransform3::ReflectYOZ(void)//YOZ面反射变换
{
	Identity();
	M[0][0] = -1;
	MultiplyMatrix();
...
void CColorTransform3::ReflectZOX(void)//ZOX面反射变换
{
	Identity();
	M[1][1] = -1;
	MultiplyMatrix();
...
void CColorTransform3::ShearX(double b, double c)//X方向错切变换
{
	Identity();
	M[0][1] = b, M[0][2] = c;
	MultiplyMatrix();
...
void CColorTransform3::ShearY(double d, double f)//Y方向错切变换
{
	Identity();
	M[1][0] = d, M[1][2] = f;
	MultiplyMatrix();
...
void CColorTransform3::ShearZ(double g, double h)//Z方向错切变换
{
	Identity();
	M[2][0] = g, M[2][1] = h;
	MultiplyMatrix();
...
void CColorTransform3::LeftMultiplyMatrix(double M1[4][4], double M2[4][4])//左乘矩阵M2 = M1 * M2
{
...
void CColorTransform3::ArbitraryDirection(double beta, double nx, double ny, double nz)//绕任意方向的旋转
