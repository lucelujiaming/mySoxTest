    ...
    uint32_t * value)
{
	...
	{
		if (IS_OPENING_TAG(apdu[0]) && value) 
		{
			*value = 0;
		} 
		else 
		{
			if (IS_CLOSING_TAG(apdu[0]) && value) 
			{
				/* closing tag */
				*value = 0;
			} 
    ...
    uint32_t * value)
{
    ...
    if (len > 0) 
	{
        ...
        if (IS_EXTENDED_VALUE(apdu[0])) 
		{
			...
			{
				if (apdu[len] == 254 && apdu_len_remaining >= 3) 
				{
					uint16_t value16;
					len++;
					len += decode_unsigned16(&apdu[len], &value16);
				...
				{
					if (apdu[len] < 254 && apdu_len_remaining >= 1) 
					{
						if (value) 
						{
							*value = apdu[len];
    ...
    BACNET_BIT_STRING * bit_string)
{
    ...
    if (len_value) 
	{
        ...
        if (bytes_used <= MAX_BITSTRING_BYTES) 
		{
            ...
            bitstring_set_bits_used(bit_string, (uint8_t) bytes_used, unused_bits);
        }
    }
    ...
    BACNET_BIT_STRING * bit_string)
{
    ...
    len = encode_tag(&apdu[0], BACNET_APPLICATION_TAG_BIT_STRING, false, bit_string_encoded_length);
    ...
    BACNET_OCTET_STRING * octet_string)
{
    ...
    if (octet_string) 
	{
        apdu_len = encode_tag(&apdu[0], BACNET_APPLICATION_TAG_OCTET_STRING, false, octetstring_length(octet_string));
