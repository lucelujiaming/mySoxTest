    ...
    char *ifname)
{
    ...
    printf("[%s:%s:%d] bip_set_socket = %d \r\n", 
                    __FILE__, __FUNCTION__, __LINE__, sock_fd);
    ...
    status =
        setsockopt(sock_fd, SOL_SOCKET, SO_REUSEADDR, &sockopt,
                                                    sizeof(sockopt));
    if (status < 0) {
        printf("[%s:%s:%d] setsockopt(SO_REUSEADDR) return %d. \r\n", 
    ...
    status =
        ...
        sizeof(sockopt));
    if (status < 0) {
        printf("[%s:%s:%d] setsockopt(SO_BROADCAST) return %d. \r\n", 
                        __FILE__, __FUNCTION__, __LINE__, status);
    ...
    printf("[%s:%s:%d] bip_get_port = %d \r\n", 
                    __FILE__, __FUNCTION__, __LINE__, bip_get_port());
    // memset(&(sin.sin_zero), '\0', sizeof(sin.sin_zero));
    status =
        bind(sock_fd, (const struct sockaddr *) &sin, sizeof(struct sockaddr));
    if (status < 0) {
        printf("[%s:%s:%d] bind(sockaddr) return %d. \r\n", 
                        __FILE__, __FUNCTION__, __LINE__, status);
        perror("bind(sockaddr) ");
        close(sock_fd);
        bip_set_socket(-1);
        return false;
    }
    ...
    void)
{
    ...
    printf("[%s:%s:%d] bip_cleanup::bip_socket = %d \n", 
