...
bool dlmstp_init(struct mstp_port_struct_t *mstp_port, 
	uint8_t mac_addr,
	uint8_t max_master,
	uint8_t max_info_frame,
	uint16_t (*silence_timer) (void),
  void (*silence_timer_reset) (void),
	void (*usart_initialize)(uint32_t baud),
	void (*usart_send_frame) (uint16_t len),
	uint16_t baud,
	uint8_t *input_buf,
	uint8_t *output_buf,
	...
	sem_t *usart_sem)
{
  ...
  dlmstp_set_mac_address(mstp_port, mac_addr);
	dlmstp_set_max_master(mstp_port, max_master);
	dlmstp_set_max_info_frames(mstp_port, max_info_frame);
	
	mstp_port->InputBuffer = input_buf;
	mstp_port->InputBufferSize = MAX_MPDU_MSTP;
	mstp_port->OutputBuffer = output_buf;
	mstp_port->OutputBufferSize = MAX_MPDU_MSTP;
	mstp_port->TransmitBuffer= transmit_buf;
	mstp_port->TransmitBufferSize= MAX_MPDU_MSTP;
	
	// point to functions
	mstp_port->SilenceTimer= silence_timer;
	mstp_port->SilenceTimerReset= silence_timer_reset;
	mstp_port->UsartInitialize= usart_initialize;
	mstp_port->UsartSendFrame= usart_send_frame;
	
	/* initialize hardware */
	mstp_port->UsartInitialize(baud);
	mstp_port->SilenceTimerReset();
	
	mstp_port->usart_sem= usart_sem;
