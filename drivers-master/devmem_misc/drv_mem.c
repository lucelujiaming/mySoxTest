...
static long memory_drv_ioctl(
           // struct inode *inode, 
           struct file *pfile, unsigned int cmd, unsigned long arg)
{
	int state = 0;
	int temp = 0;
	char *pmem = NULL;
...
static int  memory_drv_probe(struct platform_device *dev)
{
	...
	ret = misc_register(&memory_miscdrv);
	if(ret < 0)
	{
...
static int  memory_drv_remove(struct platform_device *dev)
{
	misc_deregister(&memory_miscdrv);
