...
CCabinetProjectionCube::CCabinetProjectionCube(void)
{
...
/* 这个函数给出的是长宽高都为2的立方体。                                */
/************************************************************************/
void CCabinetProjectionCube::ReadVertex(void)//点表
{
...
void CCabinetProjectionCube::ReadFace(void)//面表
{
...
void CCabinetProjectionCube::Draw(CDC* pDC)
{
	...
	for (int nFace = 0; nFace < 6; nFace++) // 面循环
	{
		...
		{
			// 和CGeometryCube直接获得投影点不同，我们这里使用斜二测投影计算投影点。
			ScreenPoint[nPoint] = projection.CabinetProjection(V[F[nFace].ptIndex[nPoint]]); // 斜投影
		}
