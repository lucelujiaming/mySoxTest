...
void COrthogonalProjectionCube::Draw(CDC* pDC)
{
	...
	for (int nFace = 0; nFace < 6; nFace++)
	{
		...
		{
			// 和CGeometryCube直接获得投影点不同，我们这里使用正交投影计算投影点。
			// 但是其实结果是一样的。就是XY不变，Z被舍弃。
			// 因此上，这里只不过是建立一个投影的概念。在后续的代码中，我们会使用其他投影方式。
			ScreenPoint[nPoint] = projection.OrthogonalProjection(V[F[nFace].ptIndex[nPoint]]);
		}
