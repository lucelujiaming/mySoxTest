...
//printf("-> %s\n", curMethod(vm, fp));
      ...
      Case CallNative:
        ...
        {
#ifdef WIN32
          printf(" CallNative::ERROR: missing  native method (%d::%d)\n"
              , *(cp+1), *(cp+2));
#else
          printf("[%s:%s:%d] CallNative::ERROR: missing  native method (%d::%d)\n"
...
// TODO - collapse code with CallNative?
      Case CallNativeWide:  
        ...
        {
#ifdef WIN32
          printf(" CallNativeWide::ERROR: missing  native method (%d::%d)\n"
             , *(cp+1), *(cp+2));
#else
          printf("[%s:%s:%d] CallNativeWide::ERROR: missing  native method (%d::%d)\n"
              , __FILE__, __FUNCTION__, __LINE__
             , *(cp+1), *(cp+2));
#endif
          return ERR_MISSING_NATIVE;
