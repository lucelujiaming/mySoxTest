...
void CLightingBicubicBezierPatch::LeftMultiplyMatrix(double M[4][4],CColorP3 P[4][4])//左乘矩阵M*P
{
...
void CLightingBicubicBezierPatch::RightMultiplyMatrix(CColorP3 P[4][4],double M[4][4])//右乘矩阵P*M
{
	CColorP3 PTemp [4][4];//临时矩阵
	for(int i = 0;i < 4;i++)
		for(int j = 0;j < 4;j++)
...
void CLightingBicubicBezierPatch::TransposeMatrix(double M[4][4])//转置矩阵
{
