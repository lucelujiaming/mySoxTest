...
void World::build()
{
	...
	noise_bunny_ptr->set_gain(0.25);
	noise_bunny_ptr->set_lacunarity(4);

	//the texture:
	FBmTextureWrapped* fBm_texture_bunny_ptr = new FBmTextureWrapped(noise_bunny_ptr);
	fBm_texture_bunny_ptr->set_color(0.7, 1.0, 0.5) ;
	fBm_texture_bunny_ptr->set_min_value(-0.1);
	fBm_texture_bunny_ptr->set_max_value(1.1);

	//the noise:
	CubicNoise* noise_box_ptr = new CubicNoise;
	noise_box_ptr->set_num_octaves(6);
	noise_box_ptr->set_gain(0.5);
	noise_box_ptr->set_lacunarity(2);

	//the texture:
	FBmTextureWrapped* fBm_texture_box_ptr = new FBmTextureWrapped(noise_box_ptr);
	fBm_texture_box_ptr->set_color(0.1, 1.0, 0.6) ;
	fBm_texture_box_ptr->set_min_value(-0.1);
	fBm_texture_box_ptr->set_max_value(1.1);
