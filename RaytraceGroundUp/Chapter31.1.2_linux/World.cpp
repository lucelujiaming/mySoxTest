...
void World::build()
{
    ...
    Checker3D *checker_bunny_ptrl = new Checker3D;
    // checker_bunny_ptrl->set_size(0.2) ;
    checker_bunny_ptrl->set_color1(purple) ;
    checker_bunny_ptrl->set_color2(white) ;
    
    InstanceTexture *instance_bunny_ptrl=new InstanceTexture(checker_bunny_ptrl);
    instance_bunny_ptrl->rotate_x(45);
    instance_bunny_ptrl->scale(2, 1, 1);

    Checker3D *checker_box_ptrl = new Checker3D;
    // checker_box_ptrl->set_size(0.2) ;
    checker_box_ptrl->set_color1(orange) ;
    checker_box_ptrl->set_color2(white) ;

    InstanceTexture *instance_box_ptrl=new InstanceTexture(checker_box_ptrl);
    instance_box_ptrl->rotate_y(45);
    instance_box_ptrl->rotate_z(45);
    instance_box_ptrl->scale(2, 2, 1);
    
    SV_Matte* sv_matte_bunny_ptrl = new SV_Matte;
    sv_matte_bunny_ptrl->set_ka(0.75) ;
    sv_matte_bunny_ptrl->set_kd(0.75) ;
    sv_matte_bunny_ptrl->set_cd(instance_bunny_ptrl) ;
    // sv_matte_bunny_ptrl->set_samples(num_samples, 0.5);

    SV_Matte* sv_matte_box_ptrl = new SV_Matte;
    sv_matte_box_ptrl->set_ka(0.75) ;
    sv_matte_box_ptrl->set_kd(0.75) ;
    sv_matte_box_ptrl->set_cd(instance_box_ptrl) ;
    // sv_matte_bunny_ptrl->set_samples(num_samples, 0.5);
	...
	Grid* bunny_ptr = new Grid(new Mesh);
	bunny_ptr->read_smooth_triangles(file_name);
	bunny_ptr->set_material(sv_matte_bunny_ptrl);
	bunny_ptr->setup_cells();

	Instance* instance_ptr = new Instance;
	//add whole grid up to this level
	instance_ptr->set_object(bunny_ptr);
	instance_ptr->set_material(sv_matte_bunny_ptrl);
	...
	box_ptr1->set_material(sv_matte_box_ptrl);
