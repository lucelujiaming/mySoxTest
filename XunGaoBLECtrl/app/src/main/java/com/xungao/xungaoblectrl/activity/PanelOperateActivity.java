...
public class PanelOperateActivity extends BaseActivity {
    ...
    private static final Integer DATA_TYPE_RW_ENUM = 0x01;
    private static final Integer DATA_TYPE_RW_INT = 0x02;
    private static final Integer DATA_TYPE_RW_FLOAT = 0x03;
    private static final Integer DATA_TYPE_RW_DOUBLE = 0x04;

    private static final Integer DATA_TYPE_RO_ITEM = 0x40;

    private static final Integer DATA_TYPE_RO_ENUM = 0x41;
    private static final Integer DATA_TYPE_RO_INT = 0x42;
    private static final Integer DATA_TYPE_RO_FLOAT = 0x43;
    private static final Integer DATA_TYPE_RO_DOUBLE = 0x44;
    ...
    private int AddPanelList(byte[] value, int iPos)
    {
        ...
        if((iPos > 0) && (iPos + 4 < value.length))
        {
            ...
            if((objPanelDeviceInfo.dataType == DATA_TYPE_RW_INT) || (objPanelDeviceInfo.dataType == DATA_TYPE_RO_INT))
            {
                System.arraycopy(value, copyPos, byteValue, 0, 4);
                objPanelDeviceInfo.value = String.valueOf(HexUtil.bytes2IntBig(byteValue));
            }
            else if((objPanelDeviceInfo.dataType == DATA_TYPE_RW_ENUM) || (objPanelDeviceInfo.dataType == DATA_TYPE_RO_ENUM)) {
                System.arraycopy(value, copyPos, byteValue, 0, 4);
                // Old Enum
                if(byteValue[1] == 0x00)
                {
            ...
            else if((objPanelDeviceInfo.dataType == DATA_TYPE_RW_FLOAT) || (objPanelDeviceInfo.dataType == DATA_TYPE_RO_FLOAT))
            {
                System.arraycopy(value, copyPos, byteValue, 0, 4);
    ...
    private void initListview() {
        ...
        mPanleInfoListview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if((device.dataType == DATA_TYPE_RW_ENUM) && (device.arrayEnumOptions != null)) {
                    ...
                    for (int i = 0; i < device.arrayEnumOptions.size(); i++) {
                        ...
                        radioButton.setButtonTintList(ColorStateList.valueOf(0xFF44BBBB));
                ...
                else if(device.dataType < DATA_TYPE_RO_ITEM ){
                    // 在Activity中创建和显示自定义对话框
                    LayoutInflater inflater = getLayoutInflater();
