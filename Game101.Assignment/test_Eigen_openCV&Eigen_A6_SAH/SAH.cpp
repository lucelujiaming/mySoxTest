...
SAHBuildNode* SAHAccel::recursiveBuild(std::vector<Object*> objects)
{
    ...
    /*   //接下来该分的只有两个，则将左右节点各一个三角形   */
		...
		std::vector<Object*> objectTmpLeft;
		objectTmpLeft.push_back(objects[0]);
		std::vector<Object*> objectTmpRight;
		objectTmpRight.push_back(objects[1]);
		node->left = recursiveBuild(objectTmpLeft);
		node->right = recursiveBuild(objectTmpRight);
        ...
        {   /*  //对三角形按照所选择的轴的位置进行排序  */
            switch (i) {
            case 0:  /*  //x轴最长，对x进行排序  */
				std::sort(objects.begin(), objects.end(), [](Object * f1, Object * f2) {return f1->getBounds().Centroid().x < f2->getBounds().Centroid().x; });
                break;
            case 1:  /*  //y轴最长，对y进行排序  */
				std::sort(objects.begin(), objects.end(), [](Object * f1, Object * f2) {return f1->getBounds().Centroid().y < f2->getBounds().Centroid().y; });
                break;
            case 2:  /*  //z轴最长，对z进行排序  */
				std::sort(objects.begin(), objects.end(), [](Object * f1, Object * f2) {return f1->getBounds().Centroid().z < f2->getBounds().Centroid().z; });
                break;
            }
            for(int j = 1; j < B; j++)
            {
