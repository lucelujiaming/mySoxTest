...
void World::build()
{
    ...
    Checker3D *checker_ptrl = new Checker3D;
    checker_ptrl->set_size(1.0) ;
    checker_ptrl->set_color1(black) ;
    checker_ptrl->set_color2(white) ;

    SV_Matte* sv_matte_ptrl = new SV_Matte;
    sv_matte_ptrl->set_ka(0.75) ;
    sv_matte_ptrl->set_kd(0.75) ;
    sv_matte_ptrl->set_cd(checker_ptrl) ;
    // sv_matte_ptrl->set_samples(num_samples, 0.5);
	...
	//Box* box_ptr1 = new Box(Point3D(0.0), Point3D(10.0));
	//box_ptr1->set_material(sv_matte_ptrl);
	//Instance* box_ptr = new Instance(box_ptr1);
	//box_ptr->translate(10, 30, 10);
	//box_ptr->scale(2, 1, 2);
	//box_ptr->rotate_x(45);
	//box_ptr->rotate_y(45);
	//box_ptr->rotate_z(45);
